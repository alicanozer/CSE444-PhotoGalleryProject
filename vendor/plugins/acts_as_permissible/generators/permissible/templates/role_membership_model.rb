class <%= role_membership_model_name %> < ActiveRecord::Base
  ...
  if roleable_type == "<%= role_model_name %>"
    if
      <%= role_model_file_name %>_id == roleable_id
      ...
      if belongs_to_itself_through_other?(roleable_id, <%= role_model_file_name %>_id)
          errors.add_to_base("A <%=
        role_model_file_name %> cannot belong to a <%= role_model_file_name %> which belongs to it.")
        end
      end
    end
  end
  ...
  def belongs_to_itself_through_other?(original_roleable_id, current_<%= role_model_file_name %>_id)
      ...
      memberships =
          self.class.find(:all, :select => "<%= role_model_file_name %>_id", :conditions => ["roleable_id=? AND roleable_type='<%= role_model_name %>'", current_<%= role_model_file_name %>_id])
